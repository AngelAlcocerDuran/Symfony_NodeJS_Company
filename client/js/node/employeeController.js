    ...
    await $.ajax({
        ...
        for(let i = 0; i < listEmployees.length; i++){
            table.append(
                ...
                "<td>"+listEmployees[i].id_office+"</th>"+
...
const getDetails = async(id) => {
    ...
    let r = new Date(employee.employee[0].registered);
    ...
    if(employee.employee[0].updated == null){
        ...
        let u = new Date(employee.employee[0].updated);
...
const getInfUpdate = async(id) => {
    ...
    document.getElementById('u_id_off').value = employee.employee[0].id_office;
}
